...
def qualitative_assessment_sem(cfg: experiment_manager.CfgNode, run_type: str = 'validation'):
    ...
    for item in tqdm(ds):
        ...
        path= "/home/antonimestre/Documents/Tesnime/SiameseSSL-master/SpaceNet7AWS/tiff_output/"+aoi_id
        ...
        imsave(path+"/y_pred_sem_t1.tif",y_pred_sem_t1.numpy())
        ...
        imsave(path+"/y_pred_sem_t2.tif",y_pred_sem_t2.numpy())
...
def qualitative_assessment_change(cfg: experiment_manager.CfgNode, run_type: str = 'test'):
    ...
    for item in tqdm(ds):
        ...
        axs[2].imshow(y_pred_change_sem.numpy(), cmap='gray')
        axs[2].set_title(r'Change Sem', fontsize=FONTSIZE)
        imsave(path+"/y_pred_change_sem.tif",y_pred_change_sem.numpy())


        y_pred_change = torch.sigmoid(logits_change).squeeze().detach()
        axs[3].imshow(y_pred_change.numpy(), cmap='gray')
        axs[3].set_title(r'Change', fontsize=FONTSIZE)
        imsave(path+"/y_pred_change.tif", y_pred_change.numpy())        
        '''
        gt_change = item['y_change'].squeeze()
        axs[4].imshow(gt_change.numpy(), cmap='gray')
        axs[4].set_title(r'GT', fontsize=FONTSIZE)
        '''
...
def quantitative_assessment(cfg: experiment_manager.CfgNode, run_type: str = 'validation'):
    ...
    net.eval()
    ds = datasets.SpaceNet7CDDataset(cfg, run_type, dataset_mode='first_last', no_augmentations=True,
                                     disable_unlabeled=False, disable_multiplier=True)
