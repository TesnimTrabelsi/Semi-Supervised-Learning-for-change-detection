...
def create_building_masks(spacenet7_path: str, aoi_id: str, save: bool = True):
    aoi_folder =   "/".join([spacenet7_path, 'train',aoi_id])
    image_folder = "/".join([aoi_folder, 'images_masked']) 
    image_folder = Path(image_folder)

    image_files = sorted([f for f in image_folder.glob('**/*')])
    for i, image_file in enumerate(image_files):
        # prepare empty mask
        img, transform, crs = geofiles.read_tif(image_file)
        ...
        buildings_file = "/".join([aoi_folder , 'labels_match_pix' , f'{image_file.stem}_Buildings.geojson'])
        feature_collection = geofiles.load_json(buildings_file)
        ...
        if save:
            save_folder = "/".join([aoi_folder , 'labels_raster'])
            save_folder=Path(save_folder)
            save_folder.mkdir(exist_ok=True)
            image_file =  "_".join([image_file.stem, "Buildings.tif"])
            #file = "/".join([str(save_folder) , image_file])

            file = save_folder / image_file
            geofiles.write_tif(file, mask, transform, crs)
            '''
            # Save upsampled images
            save_folder = "/".join([aoi_folder , f'labels_raster_X{UPSAMPLE}'])
            save_folder
            save_folder.mkdir(exist_ok=True)
...
if __name__ == '__main__':
    ...
    create_metadata_file(args.spacenet7_dir, args.dataset)
        ...
        #create_building_masks(args.spacenet7_dir, aoi_id, True)
