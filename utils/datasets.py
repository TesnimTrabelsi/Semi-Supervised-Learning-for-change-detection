...
class SpaceNet7CDDataset(AbstractSpaceNet7Dataset):
    def __init__(self, cfg: experiment_manager.CfgNode, run_type: str, no_augmentations: bool = False, dataset_mode: str = None, disable_multiplier: bool = False, disable_unlabeled: bool = True):
        ...
        if run_type == 'training':
            ...
            print("runtype is validation")
        else:
            self.aoi_ids = list(cfg.DATASET.TEST_IDS)
            print("runtype is test")   
        #changed True to False in the next instruction, when using 
        self.labeled = [False] * len(self.aoi_ids)
        self.metadata = geofiles.load_json(self.root_path / f'metadata_train.json')

        # unlabeled data for semi-supervised learning
        print(cfg.DATALOADER.INCLUDE_UNLABELED)
        print(cfg.DATALOADER.INCLUDE_UNLABELED_VALIDATION)
        print(disable_unlabeled)
    ...
    def __getitem__(self, index):
        ...
        #aoi_parts = aoi_id.split('_')
        #aoi_id=aoi_parts[0]
        #timestamps = self.metadata[aoi_id]
        # create a regex pattern to match keys starting with aoi_id
        pattern = re.compile(f"^{aoi_id}")
        # retrieve the value corresponding to the key that matches the pattern
        timestamps = next((v for k, v in self.metadata.items() if pattern.match(k)), None)
        

        timestamps = [ts for ts in timestamps if not ts['mask']]
     
        #print("#######Length of timestamp########", len(timestamps),"%%%AOID%%%:",aoi_id)
        
        if len(timestamps) > 0:
                #indices = sorted(np.random.randint(0, len(timestamps), size=3))  
                indices = [i for i in range(len(timestamps))]
                print(f"we have {len(timestamps)} for facility {aoi_id}")
  
        else:
        	 print("AOI_ID has no timestamps")
